...
for proc in "${process_count[@]}"; do
    for size in "${matrix_sizes[@]}"; do
        ...
        srun perf stat -e ${events} -o ex2_parallel_strong_sum_4_${proc}_${size}.data ../bin/ex2_parallel_sum.out -n ${proc} > ../batch_output/ex2_parallel_strong_sum_n4_${proc}_${size}_output.stdout
    done
done